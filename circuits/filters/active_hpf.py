...
def draw_plots():
    with schemdraw.Drawing() as d:
        ...
        Cf = elm.Capacitor().left(d.unit*0.5).at(op.in1).label('$C_f$')
        Ri = elm.Resistor().left(d.unit*0.5).label('$R_{i}$', loc='top').label('$v_{i}$', loc='left')
...
def draw_sliders():
    ...
    st.write(r'$K = 10$')
    st.write('### Parameters')
    st.write(r'$R_i = 1\mathrm k \Omega$')
