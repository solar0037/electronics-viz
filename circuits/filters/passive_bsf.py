...
def draw_plots():
    with schemdraw.Drawing() as d:
        V = elm.SourceV().up(d.unit*1).label('$v_{i}(t)$')
        Res = elm.Resistor().right().label('$R$')
        Cap = elm.Capacitor().down(d.unit*0.5).label('$C$')
        Ind = elm.Inductor().down(d.unit*0.5).label('$L$')
        elm.Line().to(V.start)
        plus = elm.Line().right(d.unit/2).at(Cap.start).dot(open=True)
        elm.Gap().down(d.unit*1).label(['+','$v_o$','-']).at(plus.end)
