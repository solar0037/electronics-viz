...
def draw_plots():
    with schemdraw.Drawing() as d:
        vi = elm.Line().right(d.unit/2).label('$v_{i}$', loc='left')
        elm.Line().up()
        R1 = elm.Resistor().right().label('$R$', loc='top')
        op1 = elm.Opamp(leads=True).anchor('in1')
        ...
        Ri1 = elm.Resistor().right().label('$R_i$')
        elm.Line().down()

        elm.Line().down().at(vi.end)
        ...
        Ri2 = elm.Resistor().right().label('$R_i$')
        elm.Line().up()
